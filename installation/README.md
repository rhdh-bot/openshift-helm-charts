...
curl -sSLO https://raw.githubusercontent.com/rhdh-bot/openshift-helm-charts/redhat-developer-hub-1.2-155-CI/installation/install.sh && chmod +x install.sh
./install.sh 1.2-155-CI --namespace rhdh-1-2-155-ci --chartrepo
```
...
```
oc apply -f https://github.com/rhdh-bot/openshift-helm-charts/raw/redhat-developer-hub-1.2-155-CI/installation/rhdh-next-ci-repo.yaml
```
2. Go to `Developer` perspective in your cluster
1. Select your namespace or project (eg., `rhdh-helm` or `rhdh-1-2-155-ci`)
...
podman run --rm -i -e KUBECONFIG=/.kube/config \\
  ...
  verify --write-to-file https://github.com/rhdh-bot/openshift-helm-charts/raw/redhat-developer-hub-1.2-155-CI/charts/redhat/redhat/redhat-developer-hub/1.2-155-CI/redhat-developer-hub-1.2-155-CI.tgz
